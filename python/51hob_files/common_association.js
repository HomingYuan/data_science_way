$.extend({
    commonAssociation:
    {
        ...
        getFindVendorsAssociationData: function (oSettings, sTextValue)  //调用网才接口
        {
            ...
            $.ajax({
                url: 'http://kwdsrv.51job.com/KwdSrvByKey/default.aspx',
                    ...
                    case '2':  //层级展示
                        switch (oSettings.data_struct_type)
                        {
                            case '1':  //k:v
                                if (oSettings.data_parent_click)
                                {
                                    sContent += '<span class="tl c_333"><span id="' + oSettings.association_each + '_' + i + '" data-value="' + i + '" class="' + oSettings.parent_association_click + '">' + oSettings.data[i] + '</span></span>';
                                }
                                else
                                {
                                    sContent += '<span class="tl off"><span id="' + oSettings.association_each + '_' + i + '" data-value="' + i + '">' + oSettings.data[i] + '</span></span>';
                                }
