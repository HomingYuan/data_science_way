...
# arr22 = np.arange(9)
# print('slice::', arr22[:7:2]) # 以m间距读取n个数
# print(arr22[[0, 2, 4, 6]])

# 数据展平
'''
arr23 = np.arange(24).reshape(4,6)
t = np.ravel(arr23)
k = arr23.ravel()
l = arr23.flatten()
print('t', t)  # 数组展平
...
'''
arr24 = np.arange(9).reshape(3, 3)
arr25 = arr24 * 2
print('hstack', np.hstack((arr24, arr25)))  # 横向堆积
print('vstack', np.vstack((arr24, arr25)))  # 纵向堆积
print('concatenate', np.concatenate((arr24, arr25), axis=1))  # 横向堆积
print('concatenate1', np.concatenate((arr24, arr25), axis=0))  # 纵向堆积
'''
# 分割数组
'''
arr26 = np.arange(9).reshape(3, 3)

print('hsplit', np.hsplit(arr26, 3))  # 垂直分割
print('vsplit', np.vsplit(arr26, 3))  # 水平分割
...
'''
arr27 = np.eye(2)
np.savetxt('numpy_save1.txt', arr27)

'''
# data = np.loadtxt('DataAnalyst.csv', delimiter=',') 读取数据效果比较差，建议用pandas的pd.read_csv()模块

# data = pd.read_csv('DataAnalyst.csv', delimiter=',',header=None,encoding="gb2312")  # encoding = 'gb2312' 读取中文
# print(data[1])
